@extends('layouts.app')
@section('title', 'Penjualan Reseller')
...
<div x-data="{ showModal: false }">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Penjualan Reseller</h1>
            <p class="text-gray-600">Distribusi barang jadi & pencatatan omzet</p>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div class="p-4 bg-green-50 rounded-lg border border-green-100 hover:shadow-md transition-shadow">
                        <div class="flex justify-between items-start">
                            ...
                            <div class="text-right">
                                ...
                                <span class="text-[10px] uppercase font-bold text-green-500">Siap Jual</span>
                            </div>
                        </div>
                    </div>
                    @empty
                    <div class="text-center py-8 text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-lg">
                        <p>Gudang Kosong.</p>
                        <p class="text-xs mt-1">Belum ada barang lolos QC.</p>
                    </div>
                    @endforelse
                </div>
            </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right font-bold text-red-600">- {{ $t->jumlah_keluar }}</td>
                                <td class="px-6 py-4 text-right font-mono text-gray-900 font-bold">
                                    {{ number_format($t->total_harga, 0, ',', '.') }}
                                </td>
                            </tr>
                            @empty
                            <tr><td colspan="5" class="px-6 py-8 text-center text-gray-400">Belum ada penjualan.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
    ...
    <div x-show="showModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;" x-cloak>
        ...
        <div class="relative min-h-screen flex items-center justify-center p-4">
            <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg">
                <h3 class="text-lg font-bold mb-4">Input Penjualan Baru</h3>
                
                <form action="{{ route('penjualan.store') }}" method="POST" x-data="{ stokSelected: '', qty: 0, harga: 0 }">
                    @csrf
                    <div class="space-y-4">
                        <div>
                        ...
                        <div class="bg-green-50 p-4 rounded-lg border border-green-100">
                            ...
                            <select x-model="stokSelected" class="w-full border p-2 rounded mb-3 bg-white" 
                                ...
                                ">
                                <option value="">-- Pilih Barang Siap Jual --</option>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Jumlah (Pcs)</label>
                                    <input type="number" name="jumlah" x-model="qty" min="1" required class="w-full border p-2 rounded text-center">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Harga Satuan (Rp)</label>
                                    <input type="number" name="harga" x-model="harga" min="0" required class="w-full border p-2 rounded text-center">
                                </div>
                            </div>

                            <div class="mt-3 text-right border-t border-green-200 pt-2">
                                <span class="text-xs text-gray-500">Total Estimasi:</span>
                                <div class="text-xl font-bold text-green-700 font-mono" x-text="'Rp ' + (qty * harga).toLocaleString('id-ID')">Rp 0</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 shadow-md">Simpan Transaksi</button>
                    </div>
                </form>
            </div>
        </div>
